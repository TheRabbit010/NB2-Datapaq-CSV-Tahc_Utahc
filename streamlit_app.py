...
st.set_page_config(
    page_title="Datapaq NB2 Tahc & Utahc",
    page_icon="🏭",
    layout="wide",
...
# ฟังก์ชันตรวจสอบเกณฑ์มาตรฐาน PRCNVR 02050 Rev B
def is_tahc_param_pass(val, param_type):
    ...
    try:
        if param_type in ["br_max", "dr_max"]:
            ...
            if sec is None:
                return True
            # Above 591°C: 1:30 - 4:30 min (90s - 270s)
            return 90 <= sec <= 270
            ...
            if sec is None:
                return True
            # Above 577°C: 4:00 - 7:00 min (240s - 420s) หรือ 3:30 - 5:30 min (210s - 330s) -> 210s - 420s
            return 210 <= sec <= 420

        elif param_type == "dr_250":
            sec = parse_time_to_sec(val)
            if sec is None:
                return True
            # Above 250°C: > 1:00 min (> 60s)
            return sec > 60
...
def get_probe_location(p_num, label_part=""):
    ...
    # Default fallback สำหรับ 8 Probes
    if p_num in [1, 2]:
        return "L"
    elif p_num in [3, 4]:
        return "ML"
        ...
        for r_idx, row in display_summary_df.iterrows():
            for col_tuple, param_type in col_type_mapping.items():
                if col_tuple in display_summary_df.columns:
                    val = row[col_tuple]
                    if not is_tahc_param_pass(val, param_type):
                        ng_count += 1
            ...
            for r_i, row_val in df_sum.iterrows():
                for col_t, p_type in col_type_mapping.items():
                    if col_t in df_sum.columns:
                        v = row_val[col_t]
                        if not is_tahc_param_pass(v, p_type):
